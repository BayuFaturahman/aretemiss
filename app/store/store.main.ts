...
export default class MainStore {
  ...
  async getProfileSuccess(data: ProfileModel) {
    ...
    await firebase.analytics().setUserProperties({
      ...
      nick_name: data.user_nickname,
      email: data.user_email,
      team_1: data.user_team_1_id,
      team_2: data.user_team_2_id,
      team_3: data.user_team_3_id,
      phone_number: data.user_phone_number,
      user_mood: data.user_mood,
      user_position: data.user_position ? data.user_position : '',
    })
    await firebase.analytics().setUserId(data.user_id)
  }
